...
/**
 * 【基本说明】：
 * 本文件是MobileIMSDK-Web版浏览器端SDK的主入口。
 *
 * 【网络特征】：
 * socket.io浏览器端库官方算法默认值：
 *  1）心跳ping包间隔：25秒；
 *  2）掉线后首次自动重连发生于1分钟后；
 *  3）自动重连重试间隔：5秒(有一定几率是20秒，防止服务端发生雪崩效应)。
 *
 * 【基本用法】：
 * 通过 <script src="/javascripts/libs/mobileimsdk-client-sdk.js"></script> 这样的引用就能在页面中使用了。
 *
