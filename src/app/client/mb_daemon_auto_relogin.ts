 ...
 */
export default class MBAutoReLoginDaemon {

  readonly TAG = "MBAutoReLoginDaemon";
   ...
   */
  AUTO_RE$LOGIN_INTERVAL = 3000;//2000;
   ...
   */
  setAutoReloginInterval(interval) {
    if (MBUtils.isPositiveInteger(interval)) {
      this.AUTO_RE$LOGIN_INTERVAL = interval;
    } else {
      MBUtils.mblog_w(this.TAG, "无效的AUTO_RE$LOGIN_INTERVAL参数设置：" + interval);
    }
  }
  ...
  doSendLogin() {
    ...
    if (this.mbCore.debugEnabled()) {
      MBUtils.mblog_d(this.TAG, "自动重新登陆线程执行中, autoReLogin ? " + this.mbCore.autoReLogin + "...");
    }

    let code = -1;
    // 是否允许自动重新登陆哦
    if (this.mbCore.isAutoRelogin()) {
      code = this.mbDataSender.sendLogin(this.mbCore.getCurrentLoginInfo());
    }
    return code;
  }
  ...
  onSendLogin(result) {
    ...
    // 给应用层发出尝试重新登陆/连接事件通知
    this.mbCore.emit(MBSocketEvent.SOCKET_EVENT_RECONNECT_ATTEMPT, null);
