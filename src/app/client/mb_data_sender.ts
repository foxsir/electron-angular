...
export default class MBDataSender {
  ...
  sendLogin(loginInfo) {
    ...
    if (!this.mbSocketProvider.isLocalSocketReady()) {

      if (this.mbCore.debugEnabled())
        MBUtils.mblog_d(this.TAG, "发送登陆指令时，socket连接未就绪，首先开始尝试发起连接（登陆指令将在连接成功后的回调中自动发出）。。。。");
      ...
      return this.mbSocketProvider.resetLocalSocket() != null
  ...
  sendLoginImpl(loginInfo) {
    ...
    if (code === 0) {
      ...
      this.mbCore.setCurrenetLoginInfo(loginInfo);
    }
  ...
  sendLoginout() {
    ...
    if (this.mbCore.isLoginHasInit()) {
      ...
      let p = this.mbProtocalFactory.createPLoginoutInfo(this.mbCore.getCurrentLoginInfo().loginUserId);
   ...
   */
  sendKeepAlive() {
    let p = this.mbProtocalFactory.createPKeepAlive(this.mbCore.getCurrentLoginInfo().loginUserId);
  ...
  send(dataWithString) {
    ...
    if (websocket != null && websocket.readyState === 1) {// && [ClientCoreSDK sharedInstance].connectedToServer)
      ...
      MBUtils.mblog_d(this.TAG, "scocket未连接，无法发送，本条将被忽略（dataWithString=" + dataWithString + "）!");
