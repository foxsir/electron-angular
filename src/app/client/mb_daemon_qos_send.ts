 ...
 */
export default class MBQoS4SendDaemon {

  readonly TAG = "MBQoS4SendDaemon";
  ...
  initDependent() {
    if (!this.mbCore || !this.mbDataSender) {
      this.mbCore = InstanceFactory.getInstance(MBCore);
      this.mbDataSender = InstanceFactory.getInstance(MBDataSender);
    }
  }
   ...
   * @return 已被判定为“消息未送达”的消息列表
   */
  doRetryCheck(lostMessages) {
    this.initDependent();
    ...
    try {
        ...
        if (p && p.QoS) {
          ...
          if (p.retryCount && p.retryCount >= this.QOS_TRY_COUNT) {
            if (this.mbCore.debugEnabled())
              {MBUtils.mblog_d(this.TAG, "【QoS】指纹为" + p.fp
          ...
          // 没有达到重传上限则开始进行重传
            ...
            if (delta <= this.MESSAGES_JUST$NOW_TIME) {
              if (this.mbCore.debugEnabled())
                {MBUtils.mblog_w(this.TAG, "【QoS】指纹为" + key + "的包距\"刚刚\"发出才" + delta
                  + "ms(<=" + this.MESSAGES_JUST$NOW_TIME + "ms将被认定是\"刚刚\"), 本次不需要重传哦.");}
            }
            //### 2015103 Bug Fix END
            else {
              const code = this.mbDataSender.sendCommonData(p);
              //已成功重传
              if (code === 0) {
                // 重传次数+1
                if (!p.retryCount) {
                ...
                if (this.mbCore.debugEnabled())
                  {MBUtils.mblog_d(this.TAG, "【QoS】指纹为" + p.fp + "的消息包已成功进行重传，此次之后重传次数已达" + p.retryCount + "(最多" + this.QOS_TRY_COUNT + "次).");}
              } else {
                MBUtils.mblog_w(this.TAG, "【QoS】指纹为" + p.fp + "的消息包重传失败，它的重传次数之前已累计为" + p.retryCount + "(最多" + this.QOS_TRY_COUNT + "次).");
              }
            }
          }
        }
      ...
      MBUtils.mblog_w(this.TAG, "【QoS】消息发送质量保证线程运行时发生异常：", eee);
    }
  ...
  notifyMessageLost(lostMessages) {
    ...
    this.mbCore.emit(MBSocketEvent.SOCKET_EVENT_MESSAGE_LOST, lostMessages);
  }
  ...
  put(p) {
    if (p) {
      if (p.fp) {
        ...
        MBUtils.mblog_w(this.TAG, "Invalid arg p.getFp() == null.");
      }
    } else {
      MBUtils.mblog_w(this.TAG, "Invalid arg p==null.");
    }
  }
