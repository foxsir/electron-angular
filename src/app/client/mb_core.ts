 ...
 */
export default class MBCore {

  public readonly TAG = "MBCore";

  private mbKeepAliveDaemon: MBKeepAliveDaemon;
  private mbSocketProvider: MBSocketProvider;
  private mbAutoReLoginDaemon: MBAutoReLoginDaemon;
  private mbQoS4SendDaemon: MBQoS4SendDaemon;
   ...
   */
  setConnectedToServer(connectedToServer) {
   ...
   */
  getCurrentLoginInfo() {
    return MBCore.currenetLoginInfo;
  }
   ...
   */
  setCurrenetLoginInfo(loginInfo) {
    MBCore.currenetLoginInfo = loginInfo;
  }
   ...
   */
  setCurrenetLoginInfo_firstLoginTime(firstLoginTime) {
    MBCore.currenetLoginInfo.firstLoginTime = firstLoginTime;
  }
   ...
   * 为指定的网络事件添加事件处理回调。
   *
   * 补充说明：之所以将本函数命名为“on”，是为了兼容socket.io版的MobileIMSDK-Web，
   ...
   * 抛出网络事件。
   *
   * 补充说明：之所以将本函数命名为“emite”，是为了兼容socket.io版的MobileIMSDK-Web，
  ...
  emit(eventName, data) {
    ...
    if (callbackForEvent) {
      ...
      MBUtils.mblog_w(this.TAG, '事件回调列表中没有找到eventName=' + eventName + "的callback，本次emit无法进行！");
    }
  }
  ...
  release() {
    ...
    this.mbSocketProvider = InstanceFactory.getInstance(MBSocketProvider);
    this.mbAutoReLoginDaemon = InstanceFactory.getInstance(MBAutoReLoginDaemon);
    this.mbQoS4SendDaemon = InstanceFactory.getInstance(MBQoS4SendDaemon);
    this.mbQoS4ReciveDaemon = InstanceFactory.getInstance(MBQoS4ReciveDaemon);
    ...
    this.mbQoS4SendDaemon.clear();
    // 并清除QoS接收队列缓存：防止不退出APP时切换另一账号后qos的缓存队列未清空
    this.mbQoS4ReciveDaemon.clear();
