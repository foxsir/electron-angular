 ...
 */
export default class MBKeepAliveDaemon {
  readonly TAG = "MBKeepAliveDaemon";
   ...
   */
  NETWORK_CONNECTION_TIME_OUT = this.KEEP_ALIVE_INTERVAL + 5000;//20 * 1000;//10 * 1000;

  /* 记录最近一次服务端的心跳响应包时间 */
  lastGetKeepAliveResponseFromServerTimstamp = 0;

  /* 网络断开事件观察者 */
  networkConnectionLostCallback = null;
  ...
  doKeepAlive() {
    ...
    if (this.mbCore.debugEnabled())
      MBUtils.mblog_d(this.TAG, "心跳线程执行中...");
    return this.mbDataSender.sendKeepAlive();
  }
  ...
  onKeepAlive(code) {
    ...
    // 给应用层发出ping事件通知
    this.mbCore.emit(MBSocketEvent.SOCKET_EVENT_PING, null);

    // 首先执行Keep Alive心跳包时，把此时的时间作为第1次收到服务响应的时间（初始化）
    const isInitialedForKeepAlive = (this.lastGetKeepAliveResponseFromServerTimstamp === 0);
    ...
    if (!isInitialedForKeepAlive) {
      const now = MBUtils.getCurrentUTCTimestamp();
      ...
      if (now - this.lastGetKeepAliveResponseFromServerTimstamp >= this.NETWORK_CONNECTION_TIME_OUT) {
        if (this.mbCore.debugEnabled()) {
          MBUtils.mblog_d(this.TAG, ">>>> t1=" + now + ", t2=" + this.lastGetKeepAliveResponseFromServerTimstamp
            + " -> 差：" + (now - this.lastGetKeepAliveResponseFromServerTimstamp));
          MBUtils.mblog_d(this.TAG, "心跳机制已判定网络断开，将进入断网通知和重连处理逻辑 ...");
        }
   ...
   */
  setNetworkConnectionLostCallback(networkConnectionLostCallback) {
