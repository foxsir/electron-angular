...
export default class MBDataReciever {
   ...
   */
  handleProtocal(pFromServer) {
    // if(protocalJSONString && protocalJSONString.length > 0){
    if (pFromServer) {

      try {
        // let pFromServer = JSON.parse(protocalJSONString);
        ...
        if (pFromServer.QoS) {
          ...
          if (pFromServer.type === MBProtocalType.FROM_SERVER_TYPE_OF_RESPONSE$LOGIN
            ...
            if (this.mbCore.debugEnabled())
              MBUtils.mblog_d(this.TAG, "【BugFIX】这是服务端的登陆返回响应包，且服务端判定登陆失败(即code!=0)，本次无需发送ACK应答包！");
          }
          // # Bug FIX 20170620 END 【1/2】
          else {
            // 且已经存在于接收列表中（及意味着可能是之前发给对方的应答包因网络或其它情况丢了，对方又因QoS机制重新发过来了）
            if (this.mbQoS4ReciveDaemon.hasRecieved(pFromServer.fp)) {
              if (this.mbCore.debugEnabled())
                MBUtils.mblog_d(this.TAG, "【QoS机制】" + pFromServer.fp + "已经存在于发送列表中，这是重复包，通知应用层收到该包罗！");

              //----------------------------------------- [1]代码与[2]处相同的哦 S
              // 【【C2C、C2S、S2C模式下的QoS机制2/4步：将收到的包存入QoS接收方暂存队列中（用于防重复）】】
              this.mbQoS4ReciveDaemon.addRecievedProtocal(pFromServer);
        ...
        switch (pFromServer.type) {
          ...
          default: {
            MBUtils.mblog_w(this.TAG, "收到的服务端消息类型：" + pFromServer.getType() + "，但目前该类型客户端不支持解析和处理！");
            break;
          }
        }
      } catch (e) {
        MBUtils.mblog_w(this.TAG, "处理消息的过程中发生了错误.", e);
      }
    } else {
      MBUtils.mblog_d(this.TAG, "无效的pFromServer（" + pFromServer + "）！");
    }
  }
  ...
  onRecievedCommonData(pFromServer) {
    // MBUtils.mblog_d(this.TAG, ">>>>>>>>>>>>>>>>>>>>>>>>>>>>收到"+pFromServer.getFrom()+"发过来的消息："+pFromServer.getDataContent()+".["+pFromServer.getTo()+"]");
    // 收到通用数据的回调
    this.mbCore.emit(MBSocketEvent.SOCKET_EVENT_ON_RECIEVE_MESSAGE, pFromServer);
  }
  ...
  onServerResponseKeepAlive() {
    if (this.mbCore.debugEnabled())
      MBUtils.mblog_d(this.TAG, "收到服务端回过来的Keep Alive心跳响应包.");

    // 给应用层发出pong事件通知
    this.mbCore.emit(MBSocketEvent.SOCKET_EVENT_PONG, null);

    // 更新服务端的最新响应时间（该时间将作为计算网络是否断开的依据）
    this.mbKeepAliveDaemon.updateGetKeepAliveResponseFromServerTimstamp();
  }
  ...
  onMessageRecievedACK(pFromServer) {
    ...
    this.mbQoS4SendDaemon.remove(theFingerPrint);
  }
  ...
  onServerResponseLogined(pFromServer) {
    ...
    if (loginInfoRes.code === 0) {
      ...
      if (!this.mbCore.isLoginHasInit()) {
        MBUtils.mblog_d(this.TAG, "首次登陆验证成功，firstLoginTime=" + loginInfoRes.firstLoginTime);
        // 将返回的“首次登陆时间”保存，该参数仅用于掉线重连时提交给服务端（服务端多端互踢判定逻辑中会用到）
        this.mbCore.setCurrenetLoginInfo_firstLoginTime(loginInfoRes.firstLoginTime);
      }

      this.fireConnectedToServer();
    } else {
      MBUtils.mblog_d(this.TAG, "登陆验证失败，错误码=" + loginInfoRes.code + "！");
      ...
      // 登陆失败后关闭网络监听是合理的作法
      this.mbSocketProvider.closeLocalSocket();

      // 设置中否正常连接（登陆）到服务器的标识（注意：在要event事件通知前设置哦，因为应用中是在event中处理状态的）
      this.mbCore.setConnectedToServer(false);
    }

    // 用户登陆认证情况事件通知
    // this.mbCore.emit(MBSocketEvent.SOCKET_EVENT_ON_LOGIN_RESPONSE, loginInfoRes.code);
    this.mbCore.emit(MBSocketEvent.SOCKET_EVENT_ON_LOGIN_RESPONSE, loginInfoRes);
  }
  ...
  onServerResponseError(pFromServer) {
    // 解析服务端反馈过来的消息
    let errorRes = JSON.parse(pFromServer.dataContent);
    ...
    if (errorRes.errorCode === MBErrorCode.RESPONSE_FOR_UNLOGIN) {
      //
      this.mbCore.setLoginHasInit(false);

      MBUtils.mblog_e(this.TAG, "收到服务端的“尚未登陆”的错误消息，心跳线程将停止，请应用层重新登陆.");
      // 停止心跳
      this.mbKeepAliveDaemon.stop();
      ...
      this.mbAutoReLoginDaemon.start(false);
    }

    // 收到错误响应消息的回调
    this.mbCore.emit(MBSocketEvent.SOCKET_EVENT_ON_ERROR_RESPONSE, errorRes);
  }
  ...
  onKickout(pFromServer) {
    ...
    if (this.mbCore.debugEnabled())
      MBUtils.mblog_d(this.TAG, "收到服务端发过来的“重复登录被踢”消息.");
    ...
    let kickoutInfo = JSON.parse(pFromServer.dataContent);

    /* 给应用层发出“被踢”事件通知 */
    this.mbCore.emit(MBSocketEvent.SOCKET_EVENT_KICKOUT, kickoutInfo);
    /* 给应用层发出“网络已断开”事件通知（因被踢出不是正常的掉线，所以不应走MBKeepAlive.notifyConnectionLost()这个逻辑，上
       方面的this.mbCore.release()释放掉所有通信资源后，此处就只需要简单地向ui层发出一个断网事件通知即可，方便ui刷新，别无它用） */
    this.mbCore.emit(MBSocketEvent.SOCKET_EVENT_ON_LINK_CLOSE, -1);
  }
  ...
  fireConnectedToServer() {
    ...
    this.mbKeepAliveDaemon.start(false);

    //【登陆成功后开启QoS机制】启动QoS机制之发送列表重视机制
    this.mbQoS4SendDaemon.startup(true);
    // 启动QoS机制之接收列表防重复机制
    this.mbQoS4ReciveDaemon.startup(true);
    // 设置中否正常连接（登陆）到服务器的标识（注意：在要event事件通知前设置哦，因为应用中是在event中处理状态的）
    this.mbCore.setConnectedToServer(true);
  }
...
//		QoS4SendDaemon.getInstance(context).stop();
    ...
    this.mbAutoReLoginDaemon.start(true);// 建议：此参数可由true改为false，防止服务端重启等情况下，客户端立即重连等
  }
  ...
  sendRecievedBack(pFromServer) {
    if (pFromServer.fp) {
      ...
      const pForRecivedBack = this.mbProtocalFactory.createRecivedBack(pFromServer.to, pFromServer.from, pFromServer.fp);
      const code = this.mbDataSender.sendCommonData(pForRecivedBack);

      if (this.mbCore.debugEnabled()) {
        MBUtils.mblog_d(this.TAG, "【QoS】向" + pFromServer.from + "发送" + pFromServer.fp + "包的应答包成功？(code=" + code + "),from=" + pFromServer.to + "！");
      }
    } else {
      MBUtils.mblog_w(this.TAG, "【QoS】收到" + pFromServer.from + "发过来需要QoS的包，但它的指纹码却为null！无法发应答包！");
    }
  }
}
